...
void splidar_calAEPPerChromGeneric(Splidar_OpFlag op,int minDistanceA,int maxDistanceA,int minDistanceB,int maxDistanceB,string chr,RandomAccessFile*fChrSeq,const vector<string>& fSelexa,const vector<string>& fJnxSelexa,int readMapFormat,string snipFile,string seqoutfile,string transcriptome, string jnxinfoFile,int minSpan,int readLength,bool useUniquelyMappablePosition,string uniquelyMappableChrRef,string uniquelyMappableCompactPositionFilePrefix,string GffFileName,string GBlockFile,string lociFile)
{
	...
	calculateAEP(op,minDistanceA,maxDistanceA,minDistanceB,maxDistanceB,snipFile,seqoutfile,chr,readLength,useUniquelyMappablePosition,fChrSeq);
